...
**Сделано: голос для аниме-шортсов.** Для канала видеообзоров манги нужна была озвучка моим
голосом. Сравнил три подхода, от дешёвого к дорогому:

| Подход | Что это | Обучение |
|---|---|---|
| Silero TTS v4 (ru) | готовый синтез, стоковые голоса | нет |
| XTTS | клонирование голоса по короткому образцу | нет, zero-shot |
| RVC | конверсия голоса, дообучение на моих данных | да |
...
- подготовка: нарезка на фрагменты, ресемплинг в 16 кГц, извлечение F0 и признаков, FAISS-индекс
  признаков для поиска при инференсе;
