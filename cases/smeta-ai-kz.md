...
> отчёт в Excel и Streamlit-интерфейс. RAG по СНиП РК спроектирован и стоит следующим этапом.

**Роль:** соло, от спецификации до демо · **Срок:** MVP за 10 дней, развитие после
**Стек:** Python · Claude API · pdfplumber/openpyxl · rapidfuzz · SQLite · Streamlit · pytest
...
flowchart LR
  ...
  st -.-> rag[RAG по СНиП РК<br/>в плане]
  val --> rep[/Отчёт Excel · Streamlit/]
...
Фиолетовым отмечены шаги с LLM, зелёным — детерминированные шаги. Пунктир — спроектированный, но ещё не реализованный RAG по нормативке на Qdrant.
...
Это демо для конкурса, а не продакшен. RAG по СНиП РК пока не реализован: в текущей версии сверка идёт только по базе расценок. Нормативная база в тестах синтетическая, реальный корпус
