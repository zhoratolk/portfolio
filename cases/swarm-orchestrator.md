...
## Ограничение, с которым живу

Бесплатный шлюз зависит от сети, из которой к нему обращаются. С части сетей модели Anthropic и
OpenAI отдают 403 (геоблок апстрима), а резервная модель упирается в 429. Тогда рой честно
останавливается на застое и не выдаёт полупустой результат за готовый — так было при попытке
перевести этим роем само портфолио.
